...
type InsightKindFilter = (typeof INSIGHT_KINDS)[number];

function matchesInsightFilter(
  item: InsightItem,
  filter: InsightKindFilter,
): boolean {
...
export function PageInsights() {
  const [filter, setFilter] = useState<InsightKindFilter>("All");
  const filtered = INSIGHTS.filter((i) => matchesInsightFilter(i, filter));
          ...
          <div className="insight-grid">
            {filtered.map((it, i) => (
              <div
                ...
                style={{ transitionDelay: `${String(i * 50)}ms` }}
              >
                <div className="meta">
